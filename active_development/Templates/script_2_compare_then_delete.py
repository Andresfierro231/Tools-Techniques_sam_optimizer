...
node_mult_list = [2, 6, 24] #, 48, 80, 100, 150, 300, 400, 600]

run_identifier = "temp/"
os.makedirs(run_identifier, exist_ok=False) # new directory, should not already exist
...
for node_mult in node_mult_list:
    ...
    for i in [1,2,3,4]:
        ...
        inputfile = f"jsalt{i}.i" # os.path.join("templates/", f"jsalt{i}.i")
        file = os.path.join(run_identifier, f"jsalt{i}_nodes_mult_by_{node_mult}.i")
        ...
        with open(inputfile, "r") as f_in:
            ...
            # Now append lines if node mult greater than 50 # not tested, dont know if this part works but wanted to allow smaller time steps for small node mult
            if node_mult > 50:
                with open(file, "a") as f_out:  # append mode allows smaller time steps for more refined runs
                    f_out.write("\n[Executioner]\n")
                    f_out.write("   dtmin := 1e-12\n")
                    f_out.write("[]\n")
        ...
        with open(f"{file}.txt", "a") as logf:
