...
# 
#### 
# Jadyns Paper https://docs.google.com/document/d/1_5Ws5vh5xfU7X6hZ7Zhk28DTL38MUvc8Ca32jG6ZShs/edit?usp=sharing
#############################  Experimental Prameters  ######################################################
# 


#### Control Panel ####
# Dont forget to change EOS for fluid type 
...
q_net = 189.26 # W # Note, I dont think she is using Q_heater
...
v_0 = 0.02 # 0.0185
h_amb = 1e5  # Arbitrarily large, as specified in papers
p_out = 1.1e5
...
[Postprocessors] # Will need to fix all of these... 
  ...
  [00_TS_Pr]
    type = ParsedPostprocessor
    expression = '(5806 - 10.83*TP_TS + 7.24e-3*TP_TS^2) * (0.4737 - 2.3e-3*TP_TS +3.73e-6*TP_TS^2 - 2.02e-9*TP_TS^3) / (0.78 - 1.25e-3*TP_TS +1.6e-6*TP_TS^2) '
    # pp_symbols = 'T'
    pp_names = 'TP_TS'
  []
...
[Executioner] # Creating timestepper
  ...
  ######################
  end_time = 850 # 1e3 used by Travis # I foolishly changed it to 1e6 seconds and found that it really does not change after 700 seconds for second order... Travis was right
  ######################
